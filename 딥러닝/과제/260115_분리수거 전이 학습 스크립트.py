...
datagen = ImageDataGenerator(
    ...
    rotation_range=30,      # 회전 범위 확대 (20 -> 30)
    width_shift_range=0.2,
    height_shift_range=0.2,
    shear_range=0.2,        # [추가] 이미지 기울기 조절
    zoom_range=0.2,         # [추가] 이미지 확대/축소
    horizontal_flip=True,
    fill_mode='nearest'     # [추가] 회전/이동 시 빈 공간 채우는 방식
)
...
# =============================================================================
# [2] 전이 학습 모델 설계 (Transfer Learning Model Design)
# =============================================================================
...
initial_epochs = 5 # 테스트를 위해 에폭 단축 (20 -> 5)
...
# 학습률을 1/10 ~ 1/100 수준으로 낮춰야 기존 학습된 지식이 파괴되지 않음
model.compile(loss='categorical_crossentropy',
...
plt.plot([history.epoch[-1], history.epoch[-1]], plt.ylim(), label='Start Fine Tuning', linestyle='--')
...
# 그래프 저장
graph_save_path = os.path.join(script_dir, 'training_history_finetuned.png')
