...
model_path = os.path.join(script_dir, 'best_garbage_model_improved.h5')
...
def predict_image(image_path):
    ...
    # 결과 출력
    print("\n" + "="*40)
    print(f"모델: Improved Fine-Tuned MobileNetV2")
    ...
    # 그래프 제목에 결과 표시
    plt.title(f"Improved Fine-Tuned Prediction: {predicted_class}\n({confidence:.2f}%)", fontsize=14, color='green')
    plt.axis('off')
    
    # 이미지 저장
    save_path = os.path.join(script_dir, 'prediction_result_improved.png')
