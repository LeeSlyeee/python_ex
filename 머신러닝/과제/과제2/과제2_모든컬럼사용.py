...
# =============================================================================
# [4] 변수별 상세 EDA (Detailed EDA by Feature)
# =============================================================================
# 모든 컬럼에 대해 시각화 및 분석을 수행합니다.

print("\n" + "="*80)
print("3. 변수별 상세 분석 (EDA)")
print("="*80)
...
# 범주형: object (문자열) - Class(타겟) 제외
categorical_cols = df.select_dtypes(include=['object']).columns.tolist()
if 'Class' in categorical_cols:
    categorical_cols.remove('Class')

print(f"분석할 수치형 변수: {numeric_cols}")
print(f"분석할 범주형 변수: {categorical_cols}")


# 2. 수치형 변수 시각화 (분포 및 Class 비교)
print("\n[3-1] 수치형 변수 분석 시각화 진행 중...")

# 2x2 그리드로 주요 수치형 변수 박스플롯 (기존 코드 유지 및 확장)
# 전체 수치형 변수에 대해 히스토그램과 박스플롯 저장
for col in numeric_cols:
    ...
    plt.title(f'{feature_kor_names.get(col, col)} 전체 분포')
    plt.xlabel(feature_kor_names.get(col, col))
    
    # 2. 박스플롯 (성적 등급별 분포)
    plt.subplot(1, 2, 2)
    sns.boxplot(data=df, x='Class', y=col, order=['L', 'M', 'H'], palette='Set2')
    plt.title(f'성적 등급별 {feature_kor_names.get(col, col)} 분포')
    plt.xlabel('성적 등급')
    plt.ylabel(feature_kor_names.get(col, col))
    
    plt.tight_layout()
    # 파일명에 한글이나 특수문자 없도록 처리
    save_name = f'eda_numeric_{col}.png'
    plt.savefig(os.path.join(path, save_name))
    plt.close()

print(f"  -> 수치형 변수 {len(numeric_cols)}개 그래프 저장 완료")


# 3. 범주형 변수 시각화 (빈도 및 Class 비율)
print("\n[3-2] 범주형 변수 분석 시각화 진행 중...")
...
for col in categorical_cols:
    ...
    sns.countplot(y=col, data=df, order=order_idx, palette='viridis')
    plt.title(f'{feature_kor_names.get(col, col)} 항목별 빈도')
    plt.xlabel('학생 수')
    plt.ylabel(feature_kor_names.get(col, col))
    
    # 2. 성적 등급 비율 (Stacked Bar)
    plt.subplot(1, 2, 2)
    # 교차표(Cross Tabulation) 생성
    ct = pd.crosstab(df[col], df['Class'])
    # 각 그룹별 합계로 나눠 비율 계산
    ct_ratio = ct.div(ct.sum(1), axis=0)
    # 시각화 순서 지정 (L, M, H) - 컬럼에 존재하는지 확인 후
    target_order = [cls for cls in ['L', 'M', 'H'] if cls in ct_ratio.columns]
    ct_ratio = ct_ratio[target_order]
    
    # 스택 바 차트 그리기
    ct_ratio.plot(kind='barh', stacked=True, ax=plt.gca(), colormap='coolwarm')
    plt.title(f'{feature_kor_names.get(col, col)}에 따른 성적 등급 비율')
    plt.xlabel('비율')
    plt.ylabel(feature_kor_names.get(col, col))
    plt.legend(title='Class', bbox_to_anchor=(1.05, 1), loc='upper left')
    
    plt.tight_layout()
    save_name = f'eda_categ_{col}.png'
    plt.savefig(os.path.join(path, save_name))
    plt.close()

print(f"  -> 범주형 변수 {len(categorical_cols)}개 그래프 저장 완료")
